...
models/*.yaml   ─┘    (validates each file against       (deploys to GitHub Pages)
...
`models/` — so every change is a reviewable pull request. The build validates
each file against the matching schema in `schema/` (`dataset`, `model`), writes
`site/catalog.json` and `site/models.json`, and injects the compiled records into
...
│   └── model.schema.json
├── scripts/
│   ├── build_catalog.py      # validate + compile both libraries
...
│   ├── app.js                # search + filter logic (dataset views)
...
Same workflow, different folder: copy `models/_template.yaml`, fill it in
against `schema/model.schema.json`, rebuild, and open a pull request. A
**model** is a published method with a paper (and ideally code) — pose
estimators, exposure equations, biomechanical models. When the method ships as
software, commercial or free, point `links.website` at the vendor or project
page; models can point at catalog datasets via `related_datasets`.
